...
namespace MaxyMCP.Editor.Tools
{
    ...
    internal static class ToolRegistry
    {
        ...
        public static void ScanAssemblies()
        {
                    ...
                    try
                    {
                        foreach (var type in assembly.GetTypes())
                        {
                            ...
                            foreach (var method in methods)
                            {
                                ...
                                {
                                    Debug.LogWarning($"[MaxyMCP] Duplicate tool function name: {snakeName}");
                                }
                            }
                        }
                    }
            ...
            {
                Debug.LogError($"[MaxyMCP] Error scanning assemblies for tool functions: {ex.Message}");
            }
        }
            ...
            Func<Dictionary<string, string>, string> handler)
        {
            ...
            {
                Debug.LogWarning($"[MaxyMCP] Cannot register blocked tool: {name}");
                return;
            }
            ...
            PluginDebugLogger.Log($"[MaxyMCP] Registered manual tool: {name}");
        }

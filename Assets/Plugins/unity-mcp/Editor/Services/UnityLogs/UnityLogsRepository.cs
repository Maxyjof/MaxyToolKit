...
namespace MaxyMCP.Editor.Services.UnityLogs
{
    internal class UnityLogsRepository : IDisposable
    {
        ...
        private void OnLogReceived(string message, string stackTrace, LogType type)
        {
            ...
            if (message.StartsWith("[MaxyMCP]", StringComparison.Ordinal) ||
                message.StartsWith("[MaxyMCP MCP Server]", StringComparison.Ordinal) ||

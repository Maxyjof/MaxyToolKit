...
- All 438 plugin EditMode cases passed: 421 in batch mode and the remaining 17 Recent Activity UI cases in the interactive MaxyMCP Editor. Generated skill documents and Cursor rule metadata were also validated.
...
### Added
- **MaxyMCP > MCP Settings > Recent Activity** now includes a per-project **Expand all entries by default** preference. It is enabled by default for new and previously unconfigured projects, persists across Editor reloads (including an explicit off choice), and updates the open panel immediately without restarting MCP. Manually expanded or collapsed rows retain their state until the panel is reopened.

### Changed
- Built-in `unity-mcp-workflow` and `unity-ui-composition` Project Skills are now v1.0.4. Recording guidance covers the asynchronous `record_game_view` start/status/stop lifecycle, recording IDs, finalized local-file readiness, platform and Play Mode requirements, and interrupted or inaccessible clips. UI guidance distinguishes static screenshots from short recordings for transitions and interaction sequences, without treating recording as audio or performance validation. All supported skill targets, including Cursor rules, receive the guidance and existing installations are detected as needing a skills update.
...
### Pull requests and issues
- Merged [PR #57](https://github.com/MaxyMCPAI/maxymcp-unity-mcp/pull/57): collapsible Recent Activity, with a persistent expand-all preference, readable summaries, lazy detail loading, resource cleanup, and Editor UI regression tests.
- No GitHub issues were closed by this release.

### Contributors
- Thanks [@dehuaichendragonplus](https://github.com/dehuaichendragonplus) for PR #57.
...
### Pull requests and issues
- [PR #56](https://github.com/MaxyMCPAI/maxymcp-unity-mcp/pull/56): Antigravity client configuration and Project Skills support, with workspace isolation and shared-instruction fixes.
...
### Pull requests and issues
- Merged [PR #55](https://github.com/MaxyMCPAI/maxymcp-unity-mcp/pull/55): clarified generated Claude Code guidance so an offline `maxymcp-*` entry belonging to another Unity project is not mistaken for this project's MCP server failing.
- No GitHub issues were closed by this release.
...
### Pull requests and issues
- Merged [PR #53](https://github.com/MaxyMCPAI/maxymcp-unity-mcp/pull/53): added first-class OpenCode and DeepSeek Harness configuration and Project Skills support, including the JSONC rewrite safety guard.
- Fixed and closed [Issue #54](https://github.com/MaxyMCPAI/maxymcp-unity-mcp/issues/54): `capture_scene_view` now captures Canvas UI by default and supports `include_ui=false`.
...
### Fixed
- The Claude Code entry the client config panel writes now goes into that project's `projects["<path>"].mcpServers` section of `~/.claude.json` instead of the file's top level. Claude Code applies the top level to every session on the machine regardless of which project it was opened in, so once two MaxyMCP-enabled projects had both been configured, whichever project's Unity Editor happened to be running would have its tools exposed in an unrelated project's session too — indistinguishable from that project's own entry since both use the `mcp__maxymcp-<slug>__*` namespace. Per-project naming (0.6.0) made a *call* to the wrong editor structurally impossible, but did not stop a *session* from seeing (and using) the wrong editor's tools in the first place; project-scoping the entry does. Other clients here (Cursor, VS Code, Trae, Kiro, Codex) have no equivalent scoping concept in their config file and are unaffected. The panel also now flags any `maxymcp`/`maxymcp-*` entry still sitting at the config's top level as a leftover to remove by hand, since those predate this fix and are not deleted automatically.
- A project already configured under the old top-level scheme no longer depends on someone reopening the MaxyMCP panel and clicking Configure again to stop leaking: on editor startup, `RootScopeServices` now calls a one-time, session-gated migration that moves *this project's own* previously-recorded Claude Code entry (and only that entry — proven by the same "did we record writing this key" check `RemoveSupersededMaxyMCPEntries` already uses, so another project's entry or a hand-repointed one is never touched) from the top level into its `projects["<path>"]` scope. It runs once per Editor process (not every domain reload — `~/.claude.json` can be multi-megabyte) and requires no action from the developer.
...
### Added
- Project Skills now include explicit bundled skill versions in generated `AGENTS.md`, `CLAUDE.md`, Cursor rules, and the MaxyMCP skill manifest, so users can tell when installed project guidance is stale.
- The Project Skills window now shows installed skill file status and exposes an `Upgrade Skills` action when MaxyMCP-managed guidance files are behind the package version.
- `save_to_file` option on all screenshot tools (`capture_game_view`, `capture_scene_view`, `capture_simulator_view`, `capture_multiview`): writes the PNG to disk (default `Library/MaxyMCP/Screenshots/`, or a custom `output_path` that resolves inside the Unity project root) and returns the file path instead of base64 image data. High-resolution captures previously produced multi-megabyte base64 payloads that could break MCP transports; saving to a file and letting the client read it sidesteps the payload entirely.
...
### Added
- Added a recommended `IMaxyMCPCommand` template to generated project skills, including traceable `ctx.Log` usage and Undo-aware object modification helpers.
...
### Changed
- `execute_code` now automatically adds `using MaxyMCP.Editor.Tools.Scripting;` when a full-class snippet implements an unqualified `IMaxyMCPCommand`, while avoiding duplicate usings when the namespace is already present.
...
- Moved `execute_code` safety controls out of the MCP Server window and into **MaxyMCP > MCP Settings** alongside debug logging.
...
### Fixed
- Added a project-path-hash identity to MCP `initialize` responses so an existing MaxyMCP listener can be verified as belonging to the same Unity project without exposing the raw local path.
- When HTTP binding finds the configured port already occupied, the transport now probes `initialize` and attaches only if both the MaxyMCP server name and project identity match.
...
### Added
- Added LM Studio to the MCP Server window's one-click configuration targets. The generated config writes `maxymcp` to LM Studio's `mcp.json` using Cursor-compatible `mcpServers` JSON.
...
### Added
- `MaxyMCP > MCP Server` window now shows the installed package version, polls GitHub for new releases every 6 hours, and surfaces a one-click update prompt when a newer version is available. Auto-check is skipped in Unity batch mode.
...
- New `IMaxyMCPCommand` + `ExecutionContext` API for `execute_code`. Snippets that implement `IMaxyMCPCommand` get `ctx.RegisterObjectCreation` / `RegisterObjectModification` / `DestroyObject` (auto-Undo + tracked) and `ctx.Log` / `LogWarning` / `LogError` (returned in the response).
...
- `unity-mcp-workflow` project skill rules updated to cover structured JSON returns, `instanceId` chaining, `find_method`, the new SerializedProperty-backed component setter, the IMaxyMCPCommand template, editor-state tools, and `execute_menu_item` as the preferred fallback before `execute_code`. Generated `AGENTS.md` / `CLAUDE.md` templates updated to match. Existing installed skills must be regenerated via `MaxyMCP > Project Skills` to pick up the new content.
...
- Added `MaxyMCP > Tool Exposure` for editing which tools `core` and `full` expose.
...
- Added `MaxyMCP > Plugin Settings` with a toggle for verbose plugin debug logging.
...
### Added
- Added `MaxyMCP > Project Skills` as a dedicated window for project-level skills setup
...
### Changed
- Rebranded the open-source package and documentation from GameBooom to MaxyMCP
- Moved the public Git repository to `MaxyMCPAI/maxymcp-unity-mcp`
- Updated Unity menu paths to `MaxyMCP/MCP Server` and `MaxyMCP/Check for Updates`
...
### Added
- Built-in update checking from `MaxyMCP/Check for Updates` with install-source aware behavior
...
- Package metadata now points to the `MaxyMCPAI/maxymcp-unity-mcp` repository and `0.1.4`
...
### Added
- Initial release of MaxyMCP MCP for Unity (Community Edition)

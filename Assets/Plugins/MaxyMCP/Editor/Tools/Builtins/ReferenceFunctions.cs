...
namespace MaxyMCP.Editor.Tools.Builtins
{
    ...
    internal static class ReferenceFunctions
    {
            ...
            List<object> findings, int maxResults, ref bool truncated)
        {
            ...
            try
            {
                using (var so = new SerializedObject(obj))
                {
                    ...
                    while (sp.Next(true))
                    {
                        ...
                        var hasReferenceId = sp.objectReferenceEntityIdValue.IsValid();
#else
                        var hasReferenceId = sp.objectReferenceInstanceIDValue != 0;
#endif
                        if (hasReferenceId && sp.objectReferenceValue == null)
                        {
                            if (!AddFinding(findings, maxResults, ref truncated,
            ...
            {
                Debug.LogWarning($"[MaxyMCP.References] Failed to scan '{objectPath}' ({componentName}): {ex.Message}");
            }
        }

...
namespace MaxyMCP.Editor.MCP.Server
{
    internal sealed class MCPBrokerClientTransport : IMCPTransport
    {
        ...
        public Task<bool> StartAsync(CancellationToken ct = default)
        {
            ...
            {
                Debug.LogError("[MaxyMCP MCP Server] Broker transport cannot start without a broker token.");
                return Task.FromResult(false);
            }

            if (!TryAttach())
            {
                Debug.LogError("[MaxyMCP MCP Server] Broker transport could not attach its Unity backend session.");
                return Task.FromResult(false);
            }
            ...
            PluginDebugLogger.Log("[MaxyMCP MCP Server] Broker transport attached to " + _baseUrl + "/");
            return Task.FromResult(true);
        }
        ...
        public void Stop()
        {
            ...
            _cts = null;
            PluginDebugLogger.Log("[MaxyMCP MCP Server] Broker transport stopped");
        }
        ...
        private async Task PollLoopAsync(CancellationToken ct)
        {
            while (!ct.IsCancellationRequested && _isRunning)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Debug.LogError("[MaxyMCP MCP Server] Broker request handling failed: " + ex.Message);
                }
            }
        }
        ...
        private async Task HandleAndPushAsync(BrokerPullResult pull, CancellationToken ct)
        {
            ...
            if (canPiggybackNotification && pull.AcceptsSse && MCPToolListChangeNotifier.TryConsumePending())
            {
                ...
                PluginDebugLogger.Log("[MaxyMCP MCP Server] Delivering tools/list_changed notification via broker SSE response.");
            }
            ...
            catch (Exception ex)
            {
                if (contentType != null)
                    MCPToolListChangeNotifier.RestorePending();
                Debug.LogError("[MaxyMCP MCP Server] Broker push failed: " + ex.Message);
            }
        }

...
namespace MaxyMCP.Editor.MCP.Server
{
    ...
    {
        private const string HashKey = "MaxyMCP.MCP.ExposedToolsHash";
        private const string PendingKey = "MaxyMCP.MCP.ToolsChangedPending";
        ...
        public static void CheckForChanges(MCPToolExporter toolExporter)
        {
            try
            {
                ...
                else if (!string.Equals(previous, hash, StringComparison.Ordinal))
                {
                    ...
                    PluginDebugLogger.Log(
                        "[MaxyMCP MCP Server] Exposed tool list changed; clients will be notified via tools/list_changed.");
                }
            ...
            {
                PluginDebugLogger.Log("[MaxyMCP MCP Server] Tool list change check failed: " + ex.Message);
            }
        }
